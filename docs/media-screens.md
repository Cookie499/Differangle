...
- 客户端创建或重建播放器时按服务端时间定位一次；正常播放随后由 WaterMedia 音频时钟连续驱动，不用游戏刻周期性硬跳转；
- 暂停状态每 20 tick 检查一次 WaterMedia 时钟，偏差超过 100 毫秒时校正，使所有玩家在解码稳定后停在同一进度；
- 播放、暂停、循环、音量及同一来源内的跳转会原地更新 WaterMedia 会话；暂停时保留当前纹理，目标帧解码完成后再替换，因此屏幕不会因播放器重建而清空；
- 暂停和手动跳转会建立新的时间锚点；更换来源类型或 URL 时强制重置为 0 秒；
- Differangle 使用 WaterMedia 的 `AWTEngine` 取得软件帧；CPU 侧只保留一个可复用的最新帧缓冲，GPU 侧复用同一个 `NativeImage` 和纹理，只有分辨率变化或会话关闭时才销毁并重建资源；
- 已禁用 WaterMedia 的 FFmpeg 硬件解码。实测 CUDA 路径会在约 33,520 个视频帧后停止交付画面；软件解码避免该固定帧数资源寿命问题；
- 播放期间以成功取得的软件帧作为视频心跳；当 WaterMedia 未报告暂停、加载或缓冲，播放器时钟仍持续推进，但连续 5 秒没有新视频帧时，客户端保留最后一帧、重新解析媒体来源、重建播放器，并按服务端时间恢复播放；
- 自动恢复会在日志中记录断帧时长、播放器进度、服务端目标进度和恢复次数，用于区分固定媒体时间戳、网络中断与解码线程停止；
...
服务器同步原始 URL、播放状态、基准位置和基准游戏刻，客户端各自解析、下载和解码。服务器不代理媒体字节。加入、换源、跳转和暂停以服务端时间对齐；连续播放期间允许 WaterMedia 时钟产生少量自然漂移，下一次控制操作会重新对齐。服务器离线时世界游戏时间不前进，媒体进度也不会跨停服时间推进。
...
- **已完成**：WaterMedia MP4/Bilibili 播放适配、暂停、跳转基准、循环、画质选择、固定帧缓冲与纹理复用，以及时钟推进但视频断帧时的自动恢复。
- **已完成**：WaterMedia OpenAL 声源、Minecraft 音量分类、不衰减和线性距离衰减。
- **已完成**：以外部依赖方式接入 WaterMedia 的全平台 FFmpeg 原生包和 Bilibili 平台解析。
- **已完成**：基于服务端游戏时间的多人播放进度、加入时定位，以及暂停状态的 WaterMedia 进度校正。
