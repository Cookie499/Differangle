...
2026-09-14 用户确认：复制带 NBT 的屏幕保留绑定数据，其余采用建议方案。

- 多人共享同一屏幕的 Camera UUID；服务端保存配置，仅管理员（命令权限等级 2）可创建、修改和拆除。
- 基座支持六面安装，仅基座可选取；屏幕和连接杆没有碰撞或选择框。
- 默认深灰边框和直杆，画面宽 2 格、高 1.125 格；尺寸及变换由方块实体保存。
- 右键基座打开配置界面，同时保留服务端指令。摄像机通过指令从玩家眼睛位置与朝向创建，默认隐形；可开启轴线、视锥和 UUID 调试显示。
- 首版仅使用客户端已同步的当前维度 Camera 和已加载世界内容，不主动加载远程区块，不支持跨维度画面。
- 时间单位为 tick。首版实现两点 move/look 和 step、linear、smoothstep；完整轨迹编辑、Replay、cubic 和镜子另行实现。
- **带 NBT 的屏幕复制保留 `camera_uuid` 和完整屏幕配置，仅生成新的 `screen_uuid`，不创建新 Camera 实体。** 普通放置未携带 NBT 时默认未绑定。普通存档重载保留原 Screen UUID。
- 拆除后的屏幕物品保留配置和绑定，重新放置生成新 Screen UUID；活塞移动暂时禁止。

首版使用说明与已验证范围见 [持久化资源使用说明](world-resources.md)。
